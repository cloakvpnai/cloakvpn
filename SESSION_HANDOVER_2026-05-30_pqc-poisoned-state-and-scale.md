...
| Area | Status |
|------|--------|
| Region switch "Couldn't reach Lattice" | **FIXED + DEPLOYED.** Server-side: `POST /v1/device` now provisions the new region and responds BEFORE revoking the old peer, with a re-validate guard against the A→B→A race. cloakvpn-api on `5.78.203.171`. Commits `3f87954`, `2982db0`. |
