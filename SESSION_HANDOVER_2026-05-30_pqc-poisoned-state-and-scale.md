...
| `cloak-rpd` zero-restart rosenpass daemon (scale) | **DEPLOYED FLEET-WIDE (all 10 boxes), VERIFIED.** Every box: `cloak-rpd` active, NRestarts=0, `cloak-rosenpass` inactive (unit moved to `.disabled-by-cloak-rpd`). regionsvc is socket-aware everywhere and ADDs peers live (no restart). Canary gap closed by always-ADD + idempotent add (`9f5e40c`). Real iOS client verified completing its exchange against cloak-rpd on us-east-1. Rollout tooling: `server/cloak-rpd/{rollout_box,fleet_rollout_all,canary_migrate,canary_rollback}.sh`. Per-box rollback: `bash /root/cloak-rpd-canary/canary_rollback.sh`. **Soak in progress.** |
